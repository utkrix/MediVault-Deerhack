use actix_web::{dev::Service, get, post, test, web, App, HttpResponse};
...
pub async fn dispenser_data(client: web::Data<Client>, info: web::Query<Info>) -> HttpResponse {
    ...
    match collecions.find_one(doc! {"id" : &info.id}, None).await {
...
#[post("/medicine_quantity")]
pub async fn medicine_quantity(client: web::Data<Client>, req: web::Json<MedicineDispensed>, info: web::Query<Info>) -> HttpResponse {
    println!("req {:?}", &req);
...
Medicine Vault";
        ...
        if req.time_elapsed {
            let message = MessageBuilder::new()
                .from("sr03233022@student.ku.edu.np")
                .to(vec![
                "sabinranabhat723@gmail.com",
                "sabinonweb7@gmail.com"
                ])
                .subject("Emergency Medicine Shot Required")
